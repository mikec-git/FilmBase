...
export function* getPersonDetailsSaga(action) {
  ...
  try {
    ...
    const [personDetails, combinedCredits] = yield all([
      call(axiosTMDB3, ['/person/', personId,'?api_key=', process.env.REACT_APP_TMDB_KEY, '&language=en-US'].join('')),
      call(axiosTMDB3, ['/person/', personId, '/combined_credits?api_key=', process.env.REACT_APP_TMDB_KEY, '&language=en-US'].join(''))
    ]);
